...
def main(cfg):
    ...
    for iter in range(cfg.iteration):
        ...
        for response_id in range(cfg.sample):
            ...
            indent = " " * 8
            reward_signature = "\n".join(
                [indent + line for line in reward_signature]
            )
        ...
        exec_success = False
        for response_id, (code_run, rl_run) in enumerate(
            zip(code_runs, rl_runs)
        ):
            rl_run.communicate()
            rl_filepath = f"env_iter{iter}_response{response_id}.txt"
            ...
            if traceback_msg == "":
                ...
                max_iterations = np.array(tensorboard_logs["gt_reward"]).shape[
                    0
                ]
                ...
                if (
                    "gt_reward" in tensorboard_logs
                    and "gpt_reward" in tensorboard_logs
                ):
                    gt_reward = np.array(tensorboard_logs["gt_reward"])
                    gpt_reward = np.array(tensorboard_logs["gpt_reward"])
                    reward_correlation = np.corrcoef(gt_reward, gpt_reward)[
                        0, 1
                    ]
        ...
        if max_success > max_success_overall:
            ...
            max_success_reward_correlation_overall = (
